...
```{r}
January <- dfDemand[grep("Jan", dfDemand$Date),]
January
```
...
## Create the plot
gg <- ggplot(January, mapping = aes(x=factor(Date), y = factor(ï..time.date.stamp), fill = ERCOT.Load..MW)) +
  geom_tile(colour="green") + 
  scale_fill_gradient(low = "#f4fff6", high="#0bb730") +
  scale_y_discrete(name = "Time Stamp", expand=c(0,0)) +
  scale_x_discrete(name = "Date", position = "top", expand=c(0,0)) +
  labs(title = "ERCOT Wind Over Course of Year") + 
  theme_heatmap
```
